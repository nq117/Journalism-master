...
/**
 * Created by Administrator on 2017/3/28.  社会
 */

public class SocialFragment extends Fragment {

    private final static int SOCIAL = 1;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case SOCIAL:
                    JuheBean juhe = JSON.parseObject(preferencesUtil.getString("Social", null), JuheBean.class);
        ...
        if (newses != null && NetworkState.networkConnected(getActivity()) == false) {
            for (int i = 0; i<newses.size(); i++) {
                switch (String.valueOf(newses.get(i).getName())) {
                    case "SocialFragment":
                        handler.sendEmptyMessage(SOCIAL);
                        break;
                }
            }
        } else {
    ...
    private void initData() {
        ...
        },Constant.Url.ApiSH, Constant.Common.AppKey);
    }
    ...
    private void loadGetLastest(String result) {
        ...
        if (juhe != null){
            ...
            adapter.notifyDataSetChanged();
            LiteUtils.setLite("SocialFragment", getActivity());
            String json = JSON.toJSONString(juhe);
            preferencesUtil.setString("Social", json);
        }
    }
