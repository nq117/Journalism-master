...
/**
 * Created by Administrator on 2017/3/28. 体育
 */

public class SportsFragment extends Fragment {

    private final static int SPORTS = 1;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case SPORTS:
                    JuheBean juhe = JSON.parseObject(preferencesUtil.getString("Sports", null), JuheBean.class);
        ...
        if (newses != null && NetworkState.networkConnected(getActivity()) == false) {
            for (int i = 0; i<newses.size(); i++) {
                switch (String.valueOf(newses.get(i).getName())) {
                    case "SportsFragment":
                        handler.sendEmptyMessage(SPORTS);
                        break;
                }
            }
        } else {
    ...
    private void initData() {
        ...
        }, Constant.Url.ApiTY, Constant.Common.AppKey);
    }
    ...
    private void loadGetLastest(String result) {
        ...
        if (juhe != null){
            ...
            adapter.notifyDataSetChanged();
            LiteUtils.setLite("SportsFragment", getActivity());
            String json = JSON.toJSONString(juhe);
            preferencesUtil.setString("Sports", json);
        }
    }
