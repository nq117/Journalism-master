...
/**
 * Created by Administrator on 2017/3/28. 财经
 */

public class FinanceFragment extends Fragment {

    private final static int FINANCE = 1;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case FINANCE:
                    JuheBean juhe = JSON.parseObject(preferencesUtil.getString("Finance", null), JuheBean.class);
            ...
            for (int i = 0; i<newses.size(); i++) {
                switch (String.valueOf(newses.get(i).getName())) {
                    case "FinanceFragment":
                        handler.sendEmptyMessage(FINANCE);
                        break;
                }
            }
        } else {
    ...
    private void initData() {
        ...
        }, Constant.Url.ApiCJ, Constant.Common.AppKey);
    }
    ...
    private void loadGetLastest(String result) {
        ...
        if (juhe != null){
            ...
            adapter.notifyDataSetChanged();
            LiteUtils.setLite("FinanceFragment", getActivity());
            String json = JSON.toJSONString(juhe);
            preferencesUtil.setString("Finance", json);
        }
    }
