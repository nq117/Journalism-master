...
/**
 * Created by Administrator on 2017/3/28. 军事
 */

public class MilitaryFragment extends Fragment {

    private final static int MILITARY = 1;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case MILITARY:
                    JuheBean juhe = JSON.parseObject(preferencesUtil.getString("Military", null), JuheBean.class);
        ...
        if (newses != null && NetworkState.networkConnected(getActivity()) == false) {
            for (int i = 0; i<newses.size(); i++) {
                switch (String.valueOf(newses.get(i).getName())) {
                    case "MilitaryFragment":
                        handler.sendEmptyMessage(MILITARY);
                        break;
                }
            }
        } else {
    ...
    private void initData() {
        ...
        }, Constant.Url.ApiJS, Constant.Common.AppKey);
    }
    ...
    private void loadGetLastest(String result) {
        ...
        if (juhe != null){
            ...
            adapter.notifyDataSetChanged();
            LiteUtils.setLite("MilitaryFragment", getActivity());
            String json = JSON.toJSONString(juhe);
            preferencesUtil.setString("Military", json);
        }
    }
