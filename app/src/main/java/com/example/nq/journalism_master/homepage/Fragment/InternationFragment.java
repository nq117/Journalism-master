...
/**
 * Created by Administrator on 2017/3/28. 国际
 */

public class InternationFragment extends Fragment {

    private final static int INTERNATION = 1;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case INTERNATION:
                    JuheBean juhe = JSON.parseObject(preferencesUtil.getString("Internation", null), JuheBean.class);
        ...
        if (newses != null && NetworkState.networkConnected(getActivity()) == false) {
            for (int i = 0; i<newses.size(); i++) {
                switch (String.valueOf(newses.get(i).getName())) {
                    case "InternationFragment":
                        handler.sendEmptyMessage(INTERNATION);
                        break;
                }
            }
        } else {
    ...
    private void initData() {
        ...
        }, Constant.Url.ApiGJ, Constant.Common.AppKey);
    }
    ...
    private void loadGetLastest(String result) {
        ...
        if (juhe != null){
            ...
            adapter.notifyDataSetChanged();
            LiteUtils.setLite("InternationFragment", getActivity());
            String json = JSON.toJSONString(juhe);
            preferencesUtil.setString("Internation", json);
        }
    }
