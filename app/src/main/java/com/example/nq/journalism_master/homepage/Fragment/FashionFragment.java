...
/**
 * Created by Administrator on 2017/3/28. 时尚
 */

public class FashionFragment extends Fragment {

    private final static int FASHION = 1;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case FASHION:
                    JuheBean juhe = JSON.parseObject(preferencesUtil.getString("Fashion", null), JuheBean.class);
        ...
        if (newses != null && NetworkState.networkConnected(getActivity()) == false) {
            for (int i = 0; i<newses.size(); i++) {
                switch (String.valueOf(newses.get(i).getName())) {
                    case "FashionFragment":
                        handler.sendEmptyMessage(FASHION);
                        break;
                }
            }
        } else {
    ...
    private void initData() {
        ...
        }, Constant.Url.ApiSS, Constant.Common.AppKey);
    }
    ...
    private void loadGetLastest(String result) {
        ...
        if (juhe != null){
            ...
            adapter.notifyDataSetChanged();
            LiteUtils.setLite("FashionFragment", getActivity());
            String json = JSON.toJSONString(juhe);
            preferencesUtil.setString("Fashion", json);
        }
    }
